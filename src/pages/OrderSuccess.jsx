...
import { Check, Sparkles, MapPin, Inbox, ArrowRight } from 'lucide-react';

export default function OrderSuccess() {
  const location = useLocation();
  const navigate = useNavigate();

  // Retrieve order ID from state, fallback to a mockup ID
  const orderId = location.state?.orderId || `ZIV-${Math.random().toString(36).substr(2, 9).toUpperCase()}`;
